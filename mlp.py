...
# create model
'''
hidden_layer_sizes=(10,) 指定了一个包含 10 个神经元的隐藏层。这意味着网络结构如下：

    输入层：根据鸢尾花数据集，输入层有 4 个神经元，每个神经元对应一个特征（花萼长度、花萼宽度、花瓣长度、花瓣宽度）。
    隐藏层：有 10 个神经元，由 hidden_layer_sizes=(10,) 指定。
    输出层：根据鸢尾花数据集，输出层有 3 个神经元，每个神经元对应一种鸢尾花类别（Setosa，Versicolour，Virginica）。
    random_state=42 指定了随机数生成器的种子，这可以确保每次运行代码时，模型的初始化状态相同。

    max_iter=300 指定了优化器求解模型参数的最大迭代次数。如果在达到最大迭代次数之前，模型的损失已经收敛（即，连续两次迭代损失的改进小于 tol 参数指定的阈值），则优化器将提前停止。

总结，这是一个具有单个隐藏层（10个神经元）的简单全连接神经网络模型，输入层有4个神经元，输出层有3个神经元。
'''
